...
func (p *btEpool2) get(err error, hit bool, i int, k ServiceInfo, q *d2, t *SecondaryTree, ver uint64) *SecondaryEnumerator {
...
func (q *x2) insert(i int, k ServiceInfo, ch interface{}) *x2 {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
func (t *SecondaryTree) find(q interface{}, k ServiceInfo) (i int, ok bool) {
	var mk ServiceInfo
	l := 0
	switch x := q.(type) {
	case *x2:
...
func (t *SecondaryTree) insert(q *d2, i int, k ServiceInfo, v *FinalizerList) *d2 {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
func (t *SecondaryTree) overflow(p *x2, q *d2, pi, i int, k ServiceInfo, v *FinalizerList) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *SecondaryTree) split(p *x2, q *d2, pi, i int, k ServiceInfo, v *FinalizerList) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool2.Get().(*d2)
	if q.n != nil {
		r.n = q.n
