...
func New() BTree2D {
	return &dumbmap2d{
		...
		store: make(map[string]*SecondaryLayer, 256),
	}
}

type dumbmap2d struct {
	lock  *sync.RWMutex
	store map[string]*SecondaryLayer
}

func (prev *dumbmap2d) Sync(nextMap BTree2D, onAdd, onDel func(key1 string, key2 ServiceInfo)) {
	...
	for key, layer := range prev.store {
		...
		if !ok { // deleted in the next revision
			...
			if onDel != nil {
				layer.Sync(nextLayer, nil, func(key2 ServiceInfo) {
					onDel(key, key2)
				})
			}
			continue
		...
		case onDel == nil:
			layer.Sync(nextLayer, func(key2 ServiceInfo) {
				onAdd(key, key2)
			}, nil)
		default:
			layer.Sync(nextLayer, func(key2 ServiceInfo) {
				onAdd(key, key2)
			}, func(key2 ServiceInfo) {
				onDel(key, key2)
			})
		}
	}
	...
	for key, layer := range next.store {
		...
		if !ok { // added in the next revision
			...
			if onAdd != nil {
				prevLayer.Sync(layer, func(key2 ServiceInfo) {
					onAdd(key, key2)
				}, nil)
			}
			continue
		...
		case onAdd == nil:
			prevLayer.Sync(layer, nil, func(key2 ServiceInfo) {
				onDel(key, key2)
			})
		case onDel == nil:
			prevLayer.Sync(layer, func(key2 ServiceInfo) {
...
func (b *dumbmap2d) ForEach(fn func(key string, layer SecondaryLayer) bool) {
	b.lock.RLock()
...
func (b *dumbmap2d) ForEach2(key1 string, fn func(key2 ServiceInfo) bool) {
	b.lock.RLock()
	if layer, ok := b.store[key1]; ok {
		layer.ForEach(func(k ServiceInfo, _ *FinalizerList) bool {
			return fn(k)
		})
	}
	b.lock.RUnlock()
...
func (b *dumbmap2d) SetLayer(key1 string, layer SecondaryLayer) {
	panic("not implemented")
}

func (b *dumbmap2d) GetLayer(key1 string) (SecondaryLayer, bool) {
	panic("not implemented")
}

func (b *dumbmap2d) Drop(key1 string) (ok bool) {
	b.lock.Lock()
...
func (b *dumbmap2d) Put(key1 string, key2 ServiceInfo, finalizers ...func()) {
	b.lock.Lock()
	layer, ok := b.store[key1]
	if !ok || layer == nil {
...
func (b *dumbmap2d) Delete(key1 string, key2 ServiceInfo) (ok bool) {
	b.lock.Lock()
	var layer *SecondaryLayer
	layer, ok = b.store[key1]
	if ok && layer != nil {
