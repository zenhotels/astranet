...
func (self *Registry) init() {
	self.initCtl.Do(func() {
		self.sMap = make(map[string]*Pool)
...
func (self *Registry) push(id string, srv ServiceInfo, action ...func()) {
	...
	if servicePool.srvMap == nil {
		servicePool.srvMap = make(map[ServiceInfo][]func())
	}
...
func (self *Registry) DiscoverTimeout(r Selector, sname string, wait time.Duration) (srv ServiceInfo, found bool) {
	...
	for started.Add(wait).After(now) {
		...
		if servicePool != nil {
			...
			var srvPool = make([]ServiceInfo, 0, len(servicePool.srvMap))
...
func (self *Registry) Discover(r Selector, sname string) (srv ServiceInfo, found bool) {
	self.init()
	return self.DiscoverTimeout(r, sname, time.Second)
}

func (self *Registry) Find(sname string, wait time.Duration) (servicePool *Pool) {
	self.init()
	self.rLock.RLock()
	servicePool = self.sMap[sname]
	self.rLock.RUnlock()
