...
func (t *Tree) extract(q *d, i int) { // (r Key) {
	atomic.AddUint64(&t.ver, 1)
...
func (t *Tree) Get(k Key) (v Key, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k Key, v Key) *d {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k Key, v Key) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *Tree) Put(k Key, upd func(oldV Key, exists bool) (newV Key, write bool)) (oldV Key, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV Key
...
func (t *Tree) split(p *x, q *d, pi, i int, k Key, v Key) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k Key, v Key, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k Key, v Key, err error) {
	if err = e.err; err != nil {
		return
	}
	if atomic.LoadUint64(&e.ver) != atomic.LoadUint64(&e.t.ver) {
