...
func (p *btEpool2) get(err error, hit bool, i int, k string, q *d2, t *SecondaryTree, ver uint64) *SecondaryEnumerator {
...
func (q *x2) insert(i int, k string, ch interface{}) *x2 {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
// true.
func (t *SecondaryTree) Delete(k string) (ok bool) {
	pi := -1
	var p *x2
	q := t.r
	if q == nil {
...
func (t *SecondaryTree) find(q interface{}, k string) (i int, ok bool) {
	var mk string
	l := 0
	switch x := q.(type) {
	case *x2:
...
func (t *SecondaryTree) First() (k string, v *FinalizerList) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *SecondaryTree) Get(k string) (v *FinalizerList, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *SecondaryTree) insert(q *d2, i int, k string, v *FinalizerList) *d2 {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *SecondaryTree) Last() (k string, v *FinalizerList) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *SecondaryTree) overflow(p *x2, q *d2, pi, i int, k string, v *FinalizerList) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *SecondaryTree) Seek(k string) (e *SecondaryEnumerator, ok bool) {
	q := t.r
	if q == nil {
...
func (t *SecondaryTree) Set(k string, v *FinalizerList) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *SecondaryTree) Put(k string, upd func(oldV *FinalizerList, exists bool) (newV *FinalizerList, write bool)) (oldV *FinalizerList, written bool) {
	pi := -1
	var p *x2
	q := t.r
...
func (t *SecondaryTree) split(p *x2, q *d2, pi, i int, k string, v *FinalizerList) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool2.Get().(*d2)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *SecondaryEnumerator) Next() (k string, v *FinalizerList, err error) {
...
// == io.EOF is returned.
func (e *SecondaryEnumerator) Prev() (k string, v *FinalizerList, err error) {
	if err = e.err; err != nil {
		return
	}
	if atomic.LoadUint64(&e.ver) != atomic.LoadUint64(&e.t.ver) {
