...
func (p *btEpool1) get(err error, hit bool, i int, k int, q *d1, t *PrimaryTree, ver uint64) *PrimaryEnumerator {
...
func (q *x1) insert(i int, k int, ch interface{}) *x1 {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
// true.
func (t *PrimaryTree) Delete(k int) (ok bool) {
	pi := -1
	var p *x1
	q := t.r
	if q == nil {
...
func (t *PrimaryTree) find(q interface{}, k int) (i int, ok bool) {
	var mk int
	l := 0
	switch x := q.(type) {
	case *x1:
...
func (t *PrimaryTree) First() (k int, v SecondaryLayer) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *PrimaryTree) Get(k int) (v SecondaryLayer, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *PrimaryTree) insert(q *d1, i int, k int, v SecondaryLayer) *d1 {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *PrimaryTree) Last() (k int, v SecondaryLayer) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *PrimaryTree) overflow(p *x1, q *d1, pi, i int, k int, v SecondaryLayer) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *PrimaryTree) Seek(k int) (e *PrimaryEnumerator, ok bool) {
	q := t.r
	if q == nil {
...
func (t *PrimaryTree) Set(k int, v SecondaryLayer) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *PrimaryTree) Put(k int, upd func(oldV SecondaryLayer, exists bool) (newV SecondaryLayer, write bool)) (oldV SecondaryLayer, written bool) {
	pi := -1
	var p *x1
	q := t.r
...
func (t *PrimaryTree) split(p *x1, q *d1, pi, i int, k int, v SecondaryLayer) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool1.Get().(*d1)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *PrimaryEnumerator) Next() (k int, v SecondaryLayer, err error) {
...
// == io.EOF is returned.
func (e *PrimaryEnumerator) Prev() (k int, v SecondaryLayer, err error) {
	if err = e.err; err != nil {
		return
	}
	if atomic.LoadUint64(&e.ver) != atomic.LoadUint64(&e.t.ver) {
