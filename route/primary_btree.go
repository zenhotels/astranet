...
func (p *btEpool1) get(err error, hit bool, i int, k uint64, q *d1, t *PrimaryTree, ver uint64) *PrimaryEnumerator {
...
func (q *x1) insert(i int, k uint64, ch interface{}) *x1 {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
func (t *PrimaryTree) find(q interface{}, k uint64) (i int, ok bool) {
	var mk uint64
	l := 0
	switch x := q.(type) {
	case *x1:
...
func (t *PrimaryTree) insert(q *d1, i int, k uint64, v SecondaryLayer) *d1 {
	atomic.AddUint64(&t.ver, 1)
	c := q.c
	if i < c {
...
func (t *PrimaryTree) overflow(p *x1, q *d1, pi, i int, k uint64, v SecondaryLayer) {
	atomic.AddUint64(&t.ver, 1)
	l, r := p.siblings(pi)
...
func (t *PrimaryTree) split(p *x1, q *d1, pi, i int, k uint64, v SecondaryLayer) {
	atomic.AddUint64(&t.ver, 1)
	r := btDPool1.Get().(*d1)
	if q.n != nil {
		r.n = q.n
